...
- **새로 받은 칭호가 대표가 된다**(2026-08-12): 칭호를 수여받으면 그 칭호가 자동으로 대표로 올라간다 — 방금 딴 걸 바로 자랑하게. 마음에 안 들면 본인이 컬렉션에서 다시 고르면 된다(그 선택은 덮이지 않는다). **단 같은 `ttl_group_cd`에 더 높은 등급을 이미 보유 중이면 승격하지 않는다** — 위 선택 규칙이 그런 칭호를 애초에 못 고르게 하므로, 승격까지 하면 *고를 수도 없는 칭호가 대표로 박힌다*. 정상 진행에선 하위를 나중에 받을 일이 없지만 **신규 칭호를 추가하고 sweep을 돌리면** 고인물에게 하위가 뒤늦게 붙고(실측 7명·19쌍이 전부 이 경우), **한 그룹이 동시에 들어올 때**도 이 규칙이 있어야 최고 등급이 대표가 된다(없으면 bulk INSERT의 마지막 행이 임의로 대표). 구현은 `mem_ttl_rel` INSERT 트리거 한 곳(§database-schema-v2-title-domain).
