...
- 규칙
  - INSERT 시 `vers = 1`
  - UPDATE 시 `vers = vers + 1`
  - API 업데이트 조건에 `... where id = ? and vers = ?` 포함
  - `vers` 사용 테이블은 `id + vers` 유니크 인덱스를 기본 적용한다.
- 목적
  - 동시 수정 충돌 감지
  - 감사 로그 연결 기준 제공
- 유니크 제약 기본 원칙
  - `vers` 사용 테이블은 핵심 식별 컬럼 + `vers` 조합으로 유니크를 구성한다.
...
- 팀 스코프(테넌트 격리 대상): 기록/대회운영/참가/회비 등 팀 소속 업무 데이터 전부(팀 이벤트·칭호 등은 도메인 문서에서 단계적으로 확장)
...
- 실제 기록 데이터 (`rec_race_hist`, `comp_reg_id` 강연결)
