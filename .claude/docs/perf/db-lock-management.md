...
### ✅ P0 적용 기록 (2026-06-29 완료)

**무엇을**: 3개 타임아웃을 3개 역할에 기본값으로 설정 (`postgres` 제외).

| 역할 | `lock_timeout` | `statement_timeout` | `idle_in_transaction_session_timeout` |
|------|:---:|:---:|:---:|
| `authenticated` / `anon` / `service_role` | 3s | 10s | 15s |

**어떻게**:
- 마이그레이션 파일 `supabase/migrations/20260629071954_set_role_timeouts.sql` (`ALTER ROLE … SET …`, 멱등).
- **dev**: 적용 + 검증 완료 — `pg_roles.rolconfig`로 3역할 값 확인 + `SET statement_timeout='2s'; SELECT pg_sleep(5)` 가 2초에 취소되는 것까지 시연 확인.
- **prd**: 적용 + 검증 완료 — `pg_roles.rolconfig` 3역할 값 확인. 적용 직후 postgres 로그에 timeout 에러 0건.
- PR: #372.

> 비고: dev는 `apply_migration`이 기록한 버전(`20260629071954`)과 repo 파일명을 일치시켜 재적용을 방지.
> 적용은 **새 커넥션부터** 반영(풀러가 들고 있던 기존 연결은 재접속 후).

- [x] **P0-1** `authenticated`/`anon`/`service_role` 역할에 lock/idle/statement 타임아웃 설정 (dev)
- [x] **P0-2** 역할별 실제값 검증 + 평상시 에러율 관측 (dev)
- [x] **P0-3** prd 적용
