...
**2026-09-23 갱신: 이벤트 기반 조회, 주기 폴링 없음.**

- `NotificationChannel`은 `noti_mst` Realtime 구독 대신 최초 진입·화면 복귀·푸시 수신 때
  `/api/notifications`를 백그라운드 조회한다. 알림창을 열 때도 최신 목록과 배지를 갱신한다.
- 화면 렌더는 알림 조회를 기다리지 않는다. 앱 내부 탭 이동만으로 매번 재조회하지 않는다.
- `public/sw.js`는 기존 OS 알림을 표시하면서 열린 페이지에 `NOTIFICATIONS_CHANGED` 신호를 보낸다.
  개인정보가 든 payload는 페이지에 넘기지 않고 현재 로그인 계정의 API 응답을 사용한다.
- 숨긴 페이지는 복귀할 때 조회한다. 푸시 미허용·지연 시에도 화면 복귀나 알림창 열기로 갱신된다.
  복귀 감지는 `focus`와 `visibilitychange` **둘 다** 듣는다 — 데스크톱에서 다른 앱에 갔다 오면
  탭은 계속 `visible`이라 `visibilitychange`가 안 뜨고, 탭 전환·모바일 앱 복귀는 반대다.
  대신 `RESUME_THROTTLE_MS`(30초)로 합친다: 안 그러면 알트탭·DevTools 왕복마다 service-role
  쿼리 2개짜리 조회가 나간다. ⚠️ **숨긴 동안 푸시를 놓쳤으면 이 제한을 건너뛴다**(`missedPush`) —
  안 그러면 "푸시를 받고 앱에 돌아왔는데 뱃지가 그대로"인 창이 간격만큼 넓어진다. 알림을 눌러
  들어온 경우도 마찬가지다: 같은 URL 탭이 이미 있으면 서비스워커가 `navigate` 없이 `focus`만
  해서 리마운트가 없다(§`public/sw.js`의 `handleClick` 1순위).
- 같은 화면에 계속 머무는 푸시 미허용 사용자의 배지는 자동으로 갱신되지 않는다.
- 재조회는 **새 첫 장을 앞에 놓고 그보다 오래된 장은 남긴다**(`setNotifications`) — 통째로
  갈아끼우면 무한스크롤로 내려 읽던 자리를 매번 잃는다. 경계는 `crt_at` 비교가 아니라
  **두 목록이 겹치는 마지막 자리**로 잡는다(timestamptz 문자열 폭에 기대지 않으려고).
  이어붙이지 않고 갈아끼우는 경우가 둘 있다: **첫 장이 꽉 안 찼을 때**(서버에 그게 전부라
  뒷장은 지워진 것) · **겹치는 항목이 하나도 없을 때**(그 사이 못 본 알림이 있어 이으면
  가운데가 빈 목록이 된다).
- ⚠️ 뒷장은 그 조회가 확인해 준 범위가 아니다 — 거기서 지워지거나 읽힌 건 그 장을 다시 받을
  때까지 옛 상태로 남는다. 화면 위쪽은 항상 최신이고 아래로 갈수록 오래된, 받아들인 대가다.
- 접속자 표시의 Presence/Broadcast는 별도 기능으로 유지된다. 이 변경만으로 Realtime 서비스
  전체가 꺼지지는 않으며, 실제 메모리·지연 개선 폭은 배포 후 측정해야 한다.
