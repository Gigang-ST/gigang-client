...
| `comp_evt_cfg.comp_evt_type` | (직접 문자열, 공통코드 강제 없음) | `5K`, `12K`, `HALF`, `OLYMPIC`, `GRANFONDO` 등 대회별 자유값 |
| (선택) `comp_evt_type` 참조용 | `ROAD_EVT_CD` | `5K`, `10K`, `HALF`, `FULL` |
| (선택) `comp_evt_type` 참조용 | `ULTRA_EVT_CD` | `50K`, `80K`, `100K`, `100M` |
| (선택) `comp_evt_type` 참조용 | `TRAIL_EVT_CD` | `20K`, `50K`, `100K`, `100M` |
| (선택) `comp_evt_type` 참조용 | `TRI_EVT_CD` | `SPRINT`, `OLYMPIC`, `HALF`, `FULL` |
| (선택) `comp_evt_type` 참조용 | `CYC_EVT_CD` | `GRANFONDO`, `ROAD_RACE`, `TIME_TRIAL`, `CRITERIUM` |
...
| `event_types[i]` | `comp_evt_type` | 배열 원소별 1행 생성 (자유 문자열 그대로 저장) |
...
#### A) 준비: `team_comp_plan_rel` 생성 (백필 시점 AS-IS)
| 입력 | 출력 | 규칙 |
|---|---|---|
| (`competition_id`, 기본 `team_id`) | `team_comp_plan_rel` 1건 | 중복 없이 upsert 생성 |

> **현행 앱·운영(2026-04):** 일상 운영에서는 **`team_comp_plan_rel`을 대회 카탈로그 전체에 미리 깔아 두지 않는다.** 팀원이 **참가 신청**할 때 해당 `(team_id, comp_id)`에 행이 없으면 생성하고, `comp_reg_rel`이 **한 건도 없는** 플랜 행은 제거한다(`supabase/migrations/20260419120000_team_comp_plan_rel_teammate_insert_and_prune.sql`). 관리자 **`comp_mst`만 등록**해도 플랜은 생기지 않는다. **개인 기록(`rec_race_hist`) 저장**은 팀 참가와 **연동하지 않는다.**
...
| `event_type` | `comp_evt_id` | `comp_evt_cfg(comp_id, comp_evt_type)` 조인으로 해석 |
...
| `event_type` | `comp_evt_id` | `comp_evt_type` 매칭으로 조인(불가 시 보정대상) |
