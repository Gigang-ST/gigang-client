...
### `team_utmb_prf` (팀-회원 UTMB 프로필 확장)
`utmb_profile` 확장(최근 대회 컬럼 추가) 이후 기준 테이블. 팀 컨텍스트를 포함한 UTMB 부가 정보를 관리한다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `team_utmb_prf_id` | `uuid` | Y | PK |
...
핵심 제약:
- `pk_team_utmb_prf` (`team_utmb_prf_id`)
- `fk_team_utmb_prf__team_mst` (`team_id` → `team_mst`)
- `fk_team_utmb_prf__mem_mst` (`mem_id` → `mem_mst`)
- `uk_team_utmb_prf_team_mem_vers` (`team_id`, `mem_id`, `vers`)
- `ck_team_utmb_prf_utmb_idx` (`utmb_idx >= 0`)
...
- 회원당 선택적·부가 프로필(UTMB 등)은 `mem_mst`에 직접 넣지 않고 확장 테이블(`team_utmb_prf`)에 둔다.
...
## 4) 관계/조회 모델
- 개인 프로필 조회: `mem_mst`
- UTMB 등 확장: `team_utmb_prf` (`team_id`, `mem_id`, `vers = 0`, `del_yn = false`)
