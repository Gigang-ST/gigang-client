...
**컷오버 직전·직후 마지막 확인 (dev = prd 방식):**

- **마이그레이션 정본은 Git만:** dev에만 두고 커밋하지 않은 DDL/RLS SQL이 있으면 안 된다. 운영에 반영할 것은 **저장소에 올라간 `supabase/migrations/*.sql` 전부**이며, prd는 **dev와 같은 리비전·같은 파일명 순·빠짐없이** 적용한다.
- **“같은 방식”의 의미:** 적용 도구가 CLI든 대시보드든, **실행된 마이그레이션 집합·순서가 dev 검증 리비전과 동일**하면 된다. 끝나면 **`schema_migrations.version` = 해당 `*.sql` 타임스탬프 집합**인지 반드시 대조한다.
- **데이터는 다를 수 있음:** prd와 dev의 **원본 데이터**가 다르므로 행 내용·건수가 달라지는 것은 정상이다. 여기서 맞추는 것은 **스키마·RLS·함수·트리거 등 마이그레이션이 만드는 구조**이지, 테이블 행의 동일 복제가 아니다.
...
- [x] `comp_evt_cfg` (`comp_evt_type` 자유 문자열; `20260407013000`에서 `comp_evt_cd` 공통코드 강제 제거 및 컬럼 의미 전환)
...
- [x] **컷오버 정책 교체** `20260406230000_v2_comp_admin_policy_team_role.sql`: `is_legacy_platform_admin()` 제거, `team_mem_rel.team_role_cd in ('owner','admin')` 기반으로 `comp_mst`/`comp_evt_cfg` CUD 정책 교체
...
- [x] `competition.event_types[]` → `comp_evt_cfg` (**P4** `…02204…`; **의미 전환** `…013000…` `comp_evt_type` 자유값 저장)
...
- [x] `utmb_profile` → `mem_utmb_prf` (**P9** `supabase/migrations/20260404165809_v2_mem_utmb_prf.sql`; B-4·`utmb_prf_id` 1:1)
...
| **P9** | `utmb_profile` → `mem_utmb_prf` | `mem_utmb_prf` | **`utmb_prf_id` = AS-IS `id` 1:1**; `mem_mst` 정본에 없는 `member_id` 는 미삽입 → B-4로 확인 |
...
| P4 완료 후 | 매핑된 종목만 `comp_evt_cfg`에 삽입; `ON CONFLICT DO NOTHING` 멱등; 미매핑 `event_types[]` 토큰은 집계·샘플로 리포트 | **`20260407013000` 적용 후:** 컬럼 `comp_evt_type`, 값은 공통코드 강제 없이 자유 문자열(`12K`, `HALF`, `OLYMPIC` 등) |
| P5 완료 후 | **(백필 직후 시점 검증용)** `competition` 건수 = 기본 팀(`team_id` 고정)의 `team_comp_plan_rel`(vers=0, `del_yn=false`) 건수; `comp_mst` 정본 없는 대회는 행 없음(의도 시 메모). **2026-04 `20260419120000` 고아 제거·참가 시 플랜 생성 이후에는 이 등식이 운영 DB에서 성립하지 않을 수 있다.** | `migration-map` §3.3 A); P6 `team_comp_id` 조인 전제 |
...
| P9(`mem_utmb_prf`) 완료 후 | **B-4** | `utmb_profile` 대비 `utmb_prf_id` 누락 **0건**(소스는 `mem_mst` 정본 조인 행만) |
...
| B-4 | `mem_utmb_prf` 누락 | 0건 | dev 2026-04-05: `utmb_profile` **8** = `mem_utmb_prf` 정본 **8**, B-4 **0건** (MCP) | ☑ |
...
| 2026-04-07 | `20260407013000`: `comp_evt_cfg.comp_evt_cd` → `comp_evt_type` 의미 전환(자유 문자열), `COMP_EVT_CD` 그룹 제거, 스포츠별 이벤트 코드 그룹(`ROAD_EVT_CD`~`CYC_EVT_CD`) 신설 | — |
...
| 2026-04-19 | **`team_comp_plan_rel` 운영 정책:** 참가 신청 시 플랜 생성·고아 플랜 제거·팀원 INSERT RLS — `20260419120000_team_comp_plan_rel_teammate_insert_and_prune.sql`. P5 “플랜 건수 = competition 건수” 검증은 이후 성립하지 않을 수 있음. 기록 저장은 `comp_reg_rel`과 연동하지 않음. 반영 문서: `database-schema-v2-domains.md` §2·§3, `database-schema-v2-migration-map.md` §3.3, `database-schema-v2.md` §9.2, `database-abbreviation-dictionary.md`, `database-schema-v2-app-migration-plan.md` 표 | — |
