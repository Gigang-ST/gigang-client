...
> 작성일: 2026-06-06
> 최종 수정: 2026-06-11
> 상태: 설계 중 — **홈 탭 캘린더/리스트뷰 및 sch_post(공유 일정) 부분 구현 완료**

> ⚠️ **구현 현황 주의**: 아래 "홈 탭 레이아웃"과 "리스트뷰" 섹션에 설계된 내용 중
> **캘린더/리스트 뷰 전환**, **리스트뷰 무한 스크롤**, **공유 일정(sch_post) 등록/수정/삭제**는
> `feature/home-calendar-schedule` 브랜치에서 이미 구현됨.
> 상세 진행 현황은 `.claude/docs/schedule-feature-progress.md` 참고.
>
> 아직 구현 안 된 것: `gthr_mst` 테이블 생성, 모임 생성 페이지, 모임 상세 페이지, 참석 토글, 댓글, 알림 연동
...
| 항목 | 색 | 의미 |
|------|-----|------|
| 대회 (기강 등록) | 주황 (`--warning`) | 예정된 대회 |
| 소식 | 파랑 (`--info`) | 공유 일정/정보 |
| 모임 | 핑크 (`pink-400`, `#f472b6`) | 예정된 모임 |
| 참석/참가 확정 | 초록 (`--success`) | 내가 가는 것 (대회/모임 통일) |
...
> 캘린더/리스트 뷰 전환, 공유 일정(sch_post) 등록은 구현 완료.
> 모임(gthr_mst) 연동은 미구현 — 모임 테이블 생성 후 추가 예정.

```
홈 탭
├── PageHeader
├── MiniCalendar (기본 뷰)
│   ├── 모임: 핑크 점  →  내 참석 확정: 초록 점   ← gthr_mst 미구현
│   ├── 대회: 주황 점  →  내 참가 확정: 초록 점   ← ✅ 구현됨
│   ├── 공유 일정: 파랑 점                        ← ✅ sch_post 구현됨
│   └── 헤더 왼쪽 캘린더/리스트 전환 아이콘       ← ✅ 구현됨
└── 선택된 날짜의 일정 리스트
    ├── 모임  →  참석 토글 버튼 (초록/회색)        ← gthr_mst 미구현
    └── 대회  →  기존 참가 버튼                   ← ✅ 구현됨
```

### 리스트뷰 (카톡 스타일) ✅ 일부 구현 완료

> 무한 스크롤 리스트뷰 구현 완료 (대회 + 공유 일정).
> 모임(gthr_mst) 항목은 테이블 생성 후 추가 예정.
> 구현 상세: `components/home/schedule-list-view.tsx`

```
전체 일정 (날짜순, 모임 + 대회 혼합)
├── 6.10 수  [핑크] 정기 기강권       [참석 토글]  ← gthr_mst 미구현
├── 6.14 일  [핑크] Hyrox 레츠고!    [참석 토글]  ← gthr_mst 미구현
├── 6.14 일  [파랑] 공유 일정 예시    (클릭=상세)  ← ✅ sch_post 구현됨
├── 6.14 일  [주황] Hyrox 대회       [참가 신청]  ← ✅ 구현됨
└── 6.27 토  [핑크] 잠수교 세션       [참석 토글]  ← gthr_mst 미구현
```
...
- [ ] `gthr_mst` / `gthr_attd_rel` / `gthr_cmnt_mst` 테이블 생성 (설계는 위에 있음)
- [ ] 모임 생성 페이지 (`/gatherings/new`)
- [ ] 모임 상세 페이지 (`/gatherings/[id]`) — 참석 토글, 댓글
- [ ] 홈 탭 캘린더에 모임 표시 (파랑/초록)
- [x] 홈 탭 캘린더/리스트뷰 전환 구현 ← **완료** (`feature/home-calendar-schedule`)
- [x] 홈 탭 리스트뷰 무한 스크롤 (대회 + 공유 일정) ← **완료**
- [ ] 홈 탭 리스트뷰에 모임(gthr_mst) 추가 — 테이블 생성 후
- [x] 공유 일정(sch_post) 등록/수정/삭제 ← **완료**
- [ ] 알림 설정 — 신규 일정 카테고리 추가
