...
**마일리지런은 이 목록에 없었지만 더 심했다:** `getEventGoalsCumulative`/`getEventLogsCumulative`(`lib/queries/project-data.ts`)는 `unstable_cache`에 **태그 자체가 없어서** `revalidateTag`/`updateTag` 어느 쪽도 걸 수 없는 상태였다 — 순수 `revalidate` 시간(당월 60s/지난달 3600s)에만 의존. 배경 재계산이 한 번 실패하면 재시도 장치가 없어 그대로 눌어붙는데, 실제로 환급 예정액이 몇 주째 예전 값에 멈춰있는 사고로 나타났다(2026-08-27, `evt_team_prt_rel` 참가자 보고). 처방은 같다 — `tags: ["mileage-{evtId}"]` 추가 + `logActivity`·`logActivitiesBatch`·`updateActivity`·`deleteActivity`·`joinProject`·`updateMonthlyGoal`(`app/actions/mileage-run.ts`) 전부에 `updateTag(`mileage-${evtId}`)`. **새 `unstable_cache`를 추가할 때 `tags` 없이 커밋하지 말 것** — 태그가 없으면 애초에 나중에 손쓸 방법이 없다.
