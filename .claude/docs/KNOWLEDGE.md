...
### 마일리지런 환급 예정액 — 진짜 원인은 캐시가 아니라 PostgREST 1,000행 기본 상한이었다
처음엔 `getEventGoalsCumulative`/`getEventLogsCumulative`(`lib/queries/project-data.ts`)에 태그가 없어 못 지운다고 진단하고 `tags: ["mileage-{evtId}"]` + `updateTag`를 붙여 배포했는데(#518), 그 뒤 기록을 새로 등록해도 환급 예정액이 여전히 옛 값에 멈춰 있었다(2026-08-27, 김준민·김대윤 참가자 보고). `unstable_cache` 자체를 걷어내도(#519) 안 고쳐져서 캐시 가설을 완전히 접고 다시 팠다.
**진짜 원인:** `getEventLogsCumulative`가 5개월치 활동 로그(1,401건)를 `.range()`/페이지네이션 없이 한 번에 읽고 있었는데, Supabase/PostgREST 기본 응답 상한이 **1,000행**이다 — 초과분 401건이 **에러 없이 조용히 잘렸다.** 정렬 지정도 없어 어느 참가자의 어느 달이 잘리는지 요청마다 달라질 수 있는 상태였다(들쭉날쭉하게 틀린 값이 나옴).
**이 원인을 왜 처음엔 못 찾았나:** DB 값(`evt_mlg_mth_snap.achv_mlg`)이 실제 로그 합계와 100% 일치한다고 여러 번 확인했는데, 그 검증을 전부 `execute_sql`로 **Postgres에 직접** 붙어서 했다 — 앱은 PostgREST를 거치는데 직접 SQL은 그 레이어의 1,000행 상한을 애초에 안 겪는다. "DB 데이터가 맞다"와 "앱이 그 데이터를 온전히 읽어온다"는 다른 말인데 같은 걸로 착각했다. **버그를 검증할 땐 앱이 실제로 지나가는 것과 같은 경로(PostgREST/Supabase-js)로 재현해야지, 더 낮은 레이어(직접 SQL)로 우회해서 확인하면 그 레이어에만 있는 문제를 놓친다.**
**처방:** `evt_mlg_mth_snap`/`evt_mlg_act_hist`처럼 **이벤트 전체 기간에 걸쳐 누적되는(시간이 지날수록 계속 커지는) 쿼리**는 참가자 수가 적어도 1,000행을 넘을 수 있다 — `fetchAllRows`(같은 파일)로 `.order()` + `.range()` 페이지네이션을 돌려 끝까지 읽는다. 반대로 당월 단일 달 조회(`getEventGoalsMonthly`/`getEventLogsMonthly`)는 이벤트가 길어져도 크기가 안 느는 구조라 우선순위를 낮춰 남겨뒀다. **새 쿼리를 짤 때 "이 결과가 시간이 지나며 계속 커지는가"를 먼저 물을 것** — 커진다면 `.range()` 없이는 언젠가 반드시 이 사고를 반복한다.
**곁다리로 발견한 별개 버그:** `calcMonthRefundRate`(`lib/mileage.ts`)가 `isMonthAchieved`와 다른 반올림 기준을 써서, 199.96/200처럼 "달성"으로 표시되는 달인데 환급률은 99.98%로 깎이는 어긋남이 있었다. `calcMonthRefundRate`도 `isMonthAchieved`를 그대로 호출하도록 통일해 같이 고쳤다.
...
### 허용값 CHECK를 다시 만들 땐 직전 마이그레이션이 아니라 **prd의 실제 정의**를 복사한다
`noti_mst_noti_type_enm_check`처럼 허용값 목록을 가진 CHECK는 값을 하나 더할 때마다 `DROP` →
전체 배열로 `ADD`를 다시 한다. 이때 **직전 마이그레이션 파일의 배열을 복사**하면, 그 사이 다른
파일이 더한 값이 조용히 사라진다.

- **실제 사고(2026-08-27 ~ 09-14 prd)**: `20260814130000`이 `batch_failed`를 더했는데,
  `20260825140000`(승인제 알림)이 그 이전 목록으로 CHECK를 재생성하면서 뺐다. 이후
  `20260910120000`·`20260911110000`도 그 목록을 그대로 복사했다. **18일 동안 배치 실패 알림
  INSERT가 전부 거부**됐는데 `insertNoti`가 `.catch`로 삼켜 아무도 몰랐다. 그 기간 실제 배치
  실패가 0건이라 놓친 알림은 없었다(PR #532 코드리뷰가 잡았다).
- **처방**:
  1. 목록을 쓰기 전에 prd 정의를 읽는다 —
     `select pg_get_constraintdef(oid) from pg_constraint where conname = '<제약명>';`
  2. 코드가 실제로 쓰는 값과 대조한다 —
     `grep -rhoE '(notiTypeEnm|prefTypeEnm)\s*:\s*"[a-z_0-9]+"' app lib components`
  3. `DROP`/`ADD … NOT VALID`와 `VALIDATE`는 **다른 마이그레이션 파일**로 나눈다. 한 트랜잭션이면
     `DROP`이 잡은 ACCESS EXCLUSIVE가 검증 스캔 끝까지 유지돼 `NOT VALID`로 나눈 의미가 없다
     (`supabase/migrations/README.md` "위험 변경" STEP 3/4).
- **조용한 실패의 뿌리**: 알림은 본 작업의 성공을 바꾸지 않으니 `.catch(console.error)`로 삼키는 게
  맞다. 그런데 그러면 **제약 위반 같은 영구 실패도 똑같이 조용해진다.** 새 알림 타입을 추가하면
  dev에서 그 타입이 실제로 한 건 INSERT되는 것까지 확인한다.
