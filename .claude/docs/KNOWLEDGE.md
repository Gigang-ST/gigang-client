...
**마일리지런은 이 목록에 없었지만 더 심했다:** `getEventGoalsCumulative`/`getEventLogsCumulative`(`lib/queries/project-data.ts`)는 `unstable_cache`에 **태그 자체가 없어서** `revalidateTag`/`updateTag` 어느 쪽도 걸 수 없는 상태였다 — 순수 `revalidate` 시간(당월 60s/지난달 3600s)에만 의존. 배경 재계산이 한 번 실패하면 재시도 장치가 없어 그대로 눌어붙는데, 실제로 환급 예정액이 몇 주째 예전 값에 멈춰있는 사고로 나타났다(2026-08-27, `evt_team_prt_rel` 참가자 보고). 처방은 같다 — `tags: ["mileage-{evtId}"]` 추가 + **기록(활동로그) 쓰기 액션**(`logActivity`·`logActivitiesBatch`·`updateActivity`·`deleteActivity`, `app/actions/mileage-run.ts`)에 `updateTag(`mileage-${evtId}`)`. `joinProject`(참여신청)·`updateMonthlyGoal`(목표수정)은 범위 밖으로 남겨뒀다 — 후자는 목표값(분모)을 바꾸는 드문 관리 동작이라 안 걸면 "목표 수정 직후 환급액이 늦게 반영"될 수 있다는 점은 알고 있을 것. **새 `unstable_cache`를 추가할 때 `tags` 없이 커밋하지 말 것** — 태그가 없으면 애초에 나중에 손쓸 방법이 없다.
**주의:** `updateTag`는 DB 쓰기가 **성공한 직후, 뒤따르는 재계산(`recalcGoalsFromMonth`) 실패 여부와 무관하게** 불러야 한다 — recalc의 try/catch 뒤에 두면 recalc가 실패해 조기 return될 때 DB엔 이미 반영됐는데 캐시만 안 지워지는 틈이 생긴다.
