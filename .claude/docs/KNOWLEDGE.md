...
### 마일리지런 환급 예정액 — `updateTag`로도 안 풀려서 `unstable_cache` 자체를 걷어냈다
`getEventGoalsCumulative`/`getEventLogsCumulative`(`lib/queries/project-data.ts`)에 태그가 없어 못 지운다고 처음 진단하고 `tags: ["mileage-{evtId}"]` + `updateTag`를 붙여 배포했는데(#518), 그 뒤 기록을 새로 등록해도 환급 예정액이 여전히 옛 값에 멈춰 있었다(2026-08-27, 김준민·김대윤 참가자 보고 — DB의 `achv_mlg`는 실제 로그 합계와 100% 일치, 계산식도 정상이었는데 화면만 안 맞음). 즉 **이 배포 환경에서 `updateTag`/`revalidateTag`가 `unstable_cache` 항목을 실제로 못 지우는 경우가 있다** — 기강의 전당 캐시(§`records-team-v6-*`, DESIGN.md)에서 이미 한 번 겪고 "태그 무효화 대신 캐시 키를 올려야 한다"로 우회했던 것과 같은 증상이다.
**최종 처방:** 태그를 더 정교하게 걸거나 빠진 쓰기 경로(`updateMonthlyGoal` 등)를 찾아 메우는 대신, **참가자 수십 명 규모의 가벼운 쿼리라 `unstable_cache` 자체를 뗐다** — React `cache()`(요청 내 중복 제거)만 남기고 매 요청 DB를 직접 읽는다. 돈 계산이 걸린 값은 "캐시 이득"보다 "무효화가 못 미더울 수 있다"는 리스크가 더 크다. **새 `unstable_cache`를 태그와 함께 추가하기 전에, 그 태그가 이 환경에서 실제로 지워지는지 먼저 의심할 것** — 문서상 맞는 API 호출이어도 이 프로젝트에서 두 번째로 배신당했다.
