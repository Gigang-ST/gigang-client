...
`get_member_profile` 을 통째로 막지 않는 이유: **뉴비↔모임 매칭**(러닝 프로필 조회)이 이 MCP 의 주 용도이고,
그 필드들은 앱에서 이미 공개입니다. 막아야 하는 건 앱이 공개하지 않는 두 필드뿐입니다.
...
| `list_members_attendance`가 거부됨 | 참석 현황은 owner/admin 전용입니다(§4 권한 경계). 특정 모임 미참석자(`list_gathering_non_attendees`)는 멤버도 조회할 수 있습니다. |
