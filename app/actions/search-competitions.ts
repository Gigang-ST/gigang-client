...
export async function searchCompetitions(query: string): Promise<SearchCompetition[]> {
  ...
  const { data } = await supabase
    .from("comp_mst")
    .select("comp_id, comp_nm, stt_dt, loc_nm, comp_sprt_cd, comp_evt_cfg(comp_evt_cd)")
  ...
  return (data ?? []).map((row) => ({
    ...
    event_types: (((row as unknown as { comp_evt_cfg?: { comp_evt_cd: string | null }[] }).comp_evt_cfg ?? []).map((e) => e.comp_evt_cd?.toUpperCase()).filter(Boolean) as string[]),
  }));
}
