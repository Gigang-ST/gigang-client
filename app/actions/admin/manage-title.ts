...
function parseNonNegativeInt(value: number | string, label: string): number {
  ...
  if (!Number.isInteger(n) || n < 0) throw new Error(`${label}는 0 이상의 정수여야 합니다`);
  return n;
}
...
async function normalizePayload(payload: TitlePayload): Promise<TitleNormalized> {
  ...
  if (ttlNm.length === 0) throw new Error("칭호명은 필수입니다");

  const ttlKindEnm = payload.ttlKindEnm === "auto" || payload.ttlKindEnm === "awarded" ? payload.ttlKindEnm : null;
  if (!ttlKindEnm) throw new Error("칭호 유형 값이 올바르지 않습니다");

  const ttlCtgrCd = payload.ttlCtgrCd.trim();
  if (ttlCtgrCd.length === 0) throw new Error("카테고리는 필수입니다");

  const cmmRows = await getCachedCmmCdRows();
  const validCategoryCodes = new Set(cmmCdRowsForGrp(cmmRows, "TTL_CTGR_CD").map((row) => row.cd));
  if (!validCategoryCodes.has(ttlCtgrCd)) throw new Error("유효하지 않은 카테고리입니다");
  ...
  if (payload.condRuleJson?.trim()) {
    try { condRuleJson = JSON.parse(payload.condRuleJson); }
    catch { throw new Error("자동 조건 JSON 형식이 올바르지 않습니다"); }
  } else if (ttlKindEnm === "auto") {
    throw new Error("자동 유형은 자동 조건(JSON)이 필수입니다");
  }

  const rarityLevelRaw = typeof payload.rarityLevel === "string" ? Number(payload.rarityLevel) : (payload.rarityLevel ?? 1);
  const rarityLevel = Number.isInteger(rarityLevelRaw) && rarityLevelRaw >= 1 && rarityLevelRaw <= 10 ? rarityLevelRaw : 1;

  const ttlGroupCd = payload.ttlGroupCd?.trim()
    ? (() => { const n = Number(payload.ttlGroupCd); return Number.isInteger(n) && n > 0 ? n : null; })()
    : null;
  ...
  return { ttlNm, ttlKindEnm, ttlCtgrCd, ttlDesc, descVisibility, sortOrd, useYn, condRuleJson, rarityLevel, ttlGroupCd };
}
...
export async function createTitle(payload: TitlePayload) {
  return withAdmin(async ({ member }) => {
    try {
      ...
      return { ok: false, message: error instanceof Error ? error.message : "칭호 등록에 실패했습니다" };
    }
  });
}
...
export async function updateTitle(ttlId: string, payload: TitlePayload) {
  return withAdmin(async ({ member }) => {
    try {
      ...
      return { ok: false, message: error instanceof Error ? error.message : "칭호 수정에 실패했습니다" };
    }
  });
}

export async function grantTitle(ttlId: string, teamMemId: string, rsn: string | null, isPrmy: boolean = false) {
  return withAdmin(async ({ member }) => {
    const { teamId } = await getRequestTeamContext();
    const db = createAdminClient();
    ...
    const { data: existing } = await db
      ...
      .eq("del_yn", false)
      .limit(1);
    if (existing && existing.length > 0) return { ok: false, message: "이미 보유 중인 칭호입니다" };

    if (isPrmy) {
      await db.from("mem_ttl_rel").update({ is_prmy_yn: false }).eq("team_mem_id", teamMemId).eq("is_prmy_yn", true).eq("del_yn", false);
    }
...
export async function toggleTitleUseYn(ttlId: string, useYn: boolean) {
  return withAdmin(async ({ member }) => {
    ...
    const { data, error } = await db
      .from("ttl_mst")
      .update({ use_yn: useYn, upd_by: member.id })
      .eq("ttl_id", ttlId)
      .eq("team_id", teamId)
      .eq("vers", 0)
      .eq("del_yn", false)
