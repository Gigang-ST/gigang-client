...
export async function refreshUtmbIndexes(): Promise<RefreshResult> {
  return withAdminOrThrow(async () => {
    ...
    if (memIds.length === 0) return { rows: [], summary: { updated: 0, unchanged: 0, failed: 0 } };
    ...
    for (const t of targets ?? []) {
      ...
      if (!result.ok) {
        rows.push({ memId, name, before, after: null, status: "failed", error: result.error });
      ...
      if (after === before) {
        const { error: updErr } = await admin
          ...
          .eq("mem_id", memId)
          .eq("vers", 0);
        rows.push({ memId, name, before, after, status: updErr ? "failed" : "unchanged", error: updErr?.message });
      } else {
        const { error: updErr } = await admin
          .from("mem_utmb_prf")
          .update({ utmb_idx: after, rct_race_nm: result.recentRaceName, rct_race_rec: result.recentRaceRecord })
          .eq("mem_id", memId)
          .eq("vers", 0);
        rows.push({ memId, name, before, after, status: updErr ? "failed" : "updated", error: updErr?.message });
      }
    ...
    if (summary.updated > 0) revalidateTag(`records:${teamId}`, "max");
