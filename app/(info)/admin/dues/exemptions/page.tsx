...
export default async function DuesExemptionsPage() {
  ...
  const [{ data: exemptions }, { data: hists }, { data: members }] = await Promise.all([
    ...
    db
      ...
      .select(
        "exm_cfg_id, mem_id, exm_tp_enm, exm_amt, aply_stt_dt, aply_end_dt, rsn_txt, mem_mst!fk_fee_due_exm_cfg__mem_mst(mem_nm), reg:mem_mst!fk_fee_due_exm_cfg__reg_mem_mst(mem_nm)",
      )
      .eq("team_id", teamId)
      .eq("vers", 0)
      .eq("del_yn", false)
    ...
    db
      ...
      .select(
        "exm_hist_id, mem_id, aply_ym, exm_amt, grant_src_enm, rsn_txt, mem_mst!fk_fee_due_exm_hist__mem_mst(mem_nm), aprv:mem_mst!fk_fee_due_exm_hist__aprv_mem_mst(mem_nm)",
      )
      .eq("team_id", teamId)
      .eq("vers", 0)
      .eq("del_yn", false)
    ...
    db
      .from("mem_mst")
      .select("mem_id, mem_nm, team_mem_rel!inner(team_id)")
      .eq("vers", 0)
      .eq("del_yn", false)
  ...
  const memberList = (members ?? []).map((m) => ({ mem_id: m.mem_id, mem_nm: m.mem_nm }));
