...
- **탭바 높이는 `--tabbar-h` 하나가 정본이다**(globals.css). `(main)` 본문 하단 패딩과 접속자
  레이어의 바닥선이 같은 값을 본다 — 예전엔 `calc(3.5rem + env(...))`가 레이아웃에 리터럴로
  박혀 있었는데, 그 자리에 서는 것이 둘이 되면서 한쪽만 고치면 공이 탭바에 파묻힌다.

#### 전역 접속자 레이어 — 탭바 위를 걸어다니는 사람들

지금 앱을 같이 보고 있는 사람이 하단 탭바 **윗변을 바닥 삼아** 걸어다닌다
(`components/presence/presence-layer.tsx`). 전광판 리드 위에서만 유영하던 것을 2026-09-16에
전역으로 올렸다.

- **채널 소유와 그리기를 분리한다**(`lib/presence/store.ts`). 채널은 **루트 레이아웃에 한 번**
  붙어 앱을 닫을 때까지 살고, 공은 **탭바가 있는 화면에서만** 그린다. 페이지마다 채널을 붙였다
  떼면 presence join/leave가 **접속 중인 전원에게** sync를 쏘는데, 받는 쪽마다 상태를 다시
  계산하고 목록을 리렌더한다 — 비용이 내 기기가 아니라 **남의 기기에서 인원수만큼** 불어난다.
  더 나쁜 건 눈에 보이는 결함이다: 내가 설정에 잠깐 들어갔다 나올 때마다 남들 화면에서 **내 공이
  사라졌다 새로 떨어진다.**
- **`(info)`·`(protected)`엔 안 그린다.** 거긴 탭바가 없어 바닥이 안전영역까지 내려가는데, 그
  자리가 **폼 제출 버튼이 사는 자리**다(프로필 수정·건의하기). 탭바가 있는 화면에선 탭바 자체가
  방패라 그 충돌이 없다. 관리자 표 편집·약관 같은 "일하는 화면"에 공이 굴러다닐 이유도 없다.
  판정은 `(main)` 레이아웃의 `PresenceFloor`가 store에 기록한다 — **CSS로 숨기지 않는다.**
  숨기기만 하면 rAF 루프가 계속 돈다(계산까지 멈추려면 JS가 알아야 한다).
- **`z-40`이다 — 탭바·FAB·다이얼로그(`z-50`) 아래.** 공이 FAB **뒤로** 지나가고 탭은 FAB이
  받는다. 이게 없으면 굴러다니는 32px 원이 버튼을 가려 "눌러도 안 눌리는" 화면이 된다.
  층 전체는 `pointer-events-none`이고 공(button)만 되받는다.
- **천장은 화면 끝이다.** 층은 탭바 위부터 화면 맨 위까지 차지하고, 바닥은
  `높이 − 공(32) − 이름표(13)`으로 역산한다(높이는 ResizeObserver가 잰다 — 매 프레임이 아니다).
  한때 224px 고정 띠였는데, 그건 "전력으로 튄 공 한 번이 딱 들어가는" 계산값
  (`POP_UP²/(2·GRAVITY)` ≈ 177 + 32 + 13)이라 **공중에 뜬 공을 한 번 더 치면 천장에 막혔다.**
  화면 끝까지 열면 계속 위로 올려 보낼 수 있고, 높이를 계산할 이유 자체가 사라진다.
  층이 화면을 다 덮어도 가리는 건 없다 — 배경이 없고 `pointer-events-none`이라 공 말고는
  아무것도 없는 투명한 층이고, 평소 공은 바닥 근처에만 붙어 있다.
  `overflow-hidden`은 그대로 둔다: 날아간 공이 문서 크기를 늘려 스크롤바를 만들지 않게.
- **그리는 얼굴은 30명까지**(`lib/presence/pick.ts`) — **실사용에선 안 걸리는 값이다.** 활동
  멤버가 135명이라 동시 접속이 30을 넘길 일이 사실상 없으니, 이건 "몇 명까지 보여줄까"가 아니라
  **비정상 상황의 뚜껑**이다(익명·크롤러가 몰리는 경우 등). 총원은 전광판 라벨이 말한다.
  - **공끼리 겹치는 건 막지 않는다** — 겹쳐 보이는 게 의도고, 층이 `pointer-events-none` +
    `z-40`이라 버튼을 가리지도 않는다. 한때 상한이 12였는데 "몰리면 공이 들어차 방해된다"는
    이유였고, 그 전제가 실제와 달라 근거가 성립하지 않았다. 남는 진짜 제약은 **이름표**다 —
    공은 겹쳐도 되지만 9px 글자가 포개지면 누구인지 못 읽는다.
  - **선별은 결정적이어야 한다**(랜덤이면 sync마다 얼굴이 갈려 깜빡인다): 나는 항상 포함하고
    나머지는 `mem_id` 오름차순. 회귀 테스트가 이 둘을 못박는다.
- **`IntersectionObserver`는 버렸다.** 고정 레이어는 항상 화면에 있어 관찰 대상이 없다.
  "안 보일 땐 안 움직인다"는 형태를 바꿔 남았다 — 스크롤 위치가 아니라 **이 화면이 공을 그리는
  화면인가**로 판정한다.
- **치수는 매 프레임 읽지 않는다.** `clientWidth` 읽기는 레이아웃 플러시를 강제한다 —
  ResizeObserver로 폭만 캐시하고 높이는 상수라 아예 안 잰다. 루프엔 순수 계산과
  `transform` 쓰기만 남는다(컴포지터가 처리).
- ⚠️ **느려지면 그건 용량이 아니라 버그다.** 12개 요소가 transform으로 움직이는 건 어느 기기에도
  부담이 아니다. 의심할 곳은 정해져 있다: 매 프레임 React 리렌더, 스타일 쓴 뒤 치수 읽기,
  `top/left` 이동, 매 프레임 `boxShadow`, 상한 없음, 그리고 가장 현실적인 **rAF 루프가 두 개
  도는 것**(cleanup에서 `cancelAnimationFrame`·`removeChannel`이 확실히 돌아야 한다).

**놀이와 움직임 규칙**(전역화 전부터의 것 — 그대로 유지)

- **탭하면 튀고, 그 튕김은 broadcast로 모두에게** 전해져(누구·방향·세기) 같은 공을 주고받고 서로
...
- **비로그인도 이 층을 보고 자기 얼굴도 올린다** — 단 정체 대신 유령 얼굴 + 익명 이름
  ("새벽의 페이서"). 익명 id는 sessionStorage에 담아 한 세션 내내 고정이라 색·이름·위치가 유지된다.
- **`prefers-reduced-motion`이면 아무것도 안 그린다.** 예전엔 유영 대신 얼굴을 정적으로 늘어놓았는데,
  그건 전광판 한 지면에서나 성립하던 처리다. 모든 화면 하단에 상시로 얼굴 줄이 깔리면 장식이 아니라
  방해다.
...
- **칸 모서리에 서는 건 "계기"뿐이다 — 기록의 내용이 아니라 그 기록에 대한 표시**.
  위 규칙의 예외가 아니라 그 규칙이 무엇을 막는지를 짚은 것이다: 막는 건 **사진을 작아 보이게
  하는 텍스트**(한마디·거리·날짜)이지, 사진을 안 가리는 모서리 계기가 아니다. 지금 둘이다 —
  우상단 ⚡(출처: 마일리지런 유입분) · **우하단 💬N(댓글 수)**. 판·색·크기를 같게 둬
  (`bg-black/45` + 흰 아이콘, 높이 24px) 두 배지가 한 벌로 읽히게 하고, 대각선으로 갈라 놓는다.
  새 계기를 붙이기 전에 **"사진을 눌러야 알 수 있는 것을 미리 말해 주는가"**를 먼저 묻는다.
  거리·종목·이름은 여기서 탈락한다(릴스가 맡는 내용이다).
  - **댓글 수는 0이면 안 그린다**(릴스 하단 바와 같은 규칙 — 0을 적으면 텅 빈 걸 강조하게 된다).
    prd 실측 사진 149건 중 댓글이 달린 건 26건(17.4%, 달려도 평균 1.7개)이라 여섯 칸 중 다섯은
    배지가 없는데, 그게 단점이 아니라 이 배지의 값이다 — 드무니까 "여기 대화가 붙었네"가
    눈에 띄어 그 칸을 누르게 된다. 다 켜져 있으면 배경이 된다(칭호 이력의 자동수여 라벨과 같은 이치).
  - **비로그인에게도 개수는 보인다.** `cmnt_mst`는 SELECT까지 RLS 인증 전용이지만 `get_team_posts`가
    SECURITY DEFINER라 개수만 넘어온다(본문은 아니다). 감추면 댓글이 달린 사진이 "아무도 반응
    안 한 사진"으로 오독되는데 — `RecordCommentBar` 주석이 경계하던 바로 그 상태 — 배지엔
    "로그인하면 보여요"를 적을 자리가 없다. 눌러 들어가면 릴스 하단 바가 정직하게 잇는다.
  - **숫자는 캐시(5분) 대신 릴스 실측을 덮어쓴다**: `RecordReelViewer`가 Realtime으로 읽고 있는
    개수를 `onCommentCountChange`로 격자에 올려보내고, 격자는 `resolveCommentCount(서버, 실측)`로
    합친다. `revalidateTag("story-posts")`를 부르면 **댓글 한 건이 격자 캐시 전체를 날린다**
    (응원이 태그를 안 터는 것과 같은 이유). 합성이 `??`인 건 **댓글을 다 지워 0이 됐을 때**
    `||`면 서버의 옛 값으로 되돌아가기 때문이다 — 회귀 테스트가 그 한 글자를 지킨다
    (`lib/__tests__/story-post.test.ts`).
- **기록 스와이프는 감기지 않는다**: 끝에서 더 밀어도 처음으로 돌아가지 않는다. 리드 배너는
  소식 순환이라 감기지만 여기는 시간순 목록이라, 끝이 있다는 걸 손으로 알 수 있어야 한다.
- **접속자 얼굴은 이 지면을 떠났다**(2026-09-16): 리드 위를 유영하던 아바타는 **전역 레이어**가 되어
  하단 탭바 위를 걸어다닌다(§앱 셸 — 전역 접속자 레이어). 이 지면엔 개수를 말하는
  `PresenceCount`(`지금 보는 중 N명`)만 남는다 — **인원수 카운터를 전역으로 올리지는 않는다.**
  모든 화면 하단에 상주할 이유가 없고, "이 얼굴들이 뭔가"에 답할 자리는 한 곳이면 충분하다.
  설명(`HelpTip`)은 안 붙인다: 점멸 점 + 인원수면 "지금 몇 명이 보고 있다"는 읽히고, 탭하면 튄다는 건
  한 번 눌러보면 아는 것이라 물음표를 세울 값이 아니다. 0명이면 줄째 안 그린다 — 채널이 붙기 전
  첫 렌더에도 0이라 "0명"이 잠깐 스쳤다 사라지는 깜빡임이 생긴다.
  - 얼굴이 빠지면서 이 지면의 옛 제약이 **셋 다** 풀렸다: 아바타가 걸어다닐 바닥을 확보하려고 리드
    아래에 이름표 몫까지 더해 두던 패딩, 라벨을 바닥선 위에 겹쳐 띄우던 `BADGE_LIFT`, 그리고 매 프레임
    transform을 찍는 자식 때문에 필요했던 `z-0` 스태킹 컨텍스트 격리 — **격리할 자식이 없어졌다.**
...
- **검색창은 걷어냈다.** 가장 긴 목록이 21행(실측 하프 남자)이라 이름을 못 찾을 일이 없다.
  그 자리를 판독선이 가져갔다.
- **크루 가입일 이전 기록은 랭킹에 서지 않는다**(`get_public_team_race_rankings`의
  `race_dt >= team_mem_rel.join_dt`, 2026-09-08 #528). 전당은 "기강에서 함께 쌓은 기록"의 판이라
  입단 전 전성기 기록이 순위를 만들면 안 된다. 다만 **등록·보관·프로필 노출은 그대로다** — 자기
  과거 이력을 올리는 건 의도된 동작이고, 거르는 곳은 랭킹 RPC 한 곳뿐이다(랭킹 화면과
  `save-race-record`의 TOP10 캐시무효화 판정이 이 RPC를 공유하므로 기준이 갈리지 않는다).
  가입 당일 대회는 인정(`>=`), `join_dt`가 비면 거르지 않는다(fail-open — 가입일 누락으로 기록이
  조용히 사라지는 쪽이 더 나쁘다), 재가입자는 `vers=0` 한 행이라 최초 가입일 기준이다.
  적용하면서 캐시 키를 `v7`로 올렸다 — 페이로드 모양은 그대로지만 옛 캐시의 순위가 24시간 남는 걸 끊기 위해서다.
  ⚠️ 랭킹에서 빠지기만 하고 **본인에게 안내가 없다** — 기록 목록에 「가입 전 기록」 표시를 붙일지는
  별건이다.
- **카테고리는 `SegmentControl`, 종목은 pill.** 둘 다 pill이던 시절엔 나란히 뜬 두 줄이 같은 층으로
  읽혔다. 형태를 갈라 층을 만든다.
- 캐시 키는 `records-team-v7-*` — 페이로드 모양을 바꾸면 **키를 올린다**. 안 올리면 배포 직후 남아
