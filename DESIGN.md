...
- **카테고리는 `SegmentControl`, 종목은 pill.** 둘 다 pill이던 시절엔 나란히 뜬 두 줄이 같은 층으로
  읽혔다. 형태를 갈라 층을 만든다.
- 캐시 키는 `records-team-v5-*` — 페이로드 모양을 바꾸면 **키를 올린다**. 안 올리면 배포 직후 남아
  있는 옛 캐시가 새 필드 없이 돌아와 띠·판독선이 조용히 빈다. 한마디를 고치면
  `setIntroTxt`가 `records:${teamId}` 태그를 턴다(안 그러면 전당에만 옛 말이 24시간 남는다).
