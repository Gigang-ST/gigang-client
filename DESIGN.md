...
- **띠는 224px로 높다.** 바닥은 `높이 − 공(32) − 이름표(13)`라, 높이가 공+이름표와 같으면 바닥이
  0이 되어 **탭해도 공이 안 뜬다**(천장에 즉시 막힌다). 튕김 최고점은 물리 상수가 정한다
  (`POP_UP²/(2·GRAVITY)` ≈ 177px). 배경이 없고 포인터를 통과시키므로 높아도 화면을 안 가린다.
