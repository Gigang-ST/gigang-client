...
- **그리는 얼굴은 12명까지**(`lib/presence/pick.ts`). 성능이 아니라 **크라우딩** 대비다 — 활동
  멤버가 135명이고 모임 대기열 푸시가 나가면 몇 초 안에 동시에 들어오는데, 하필 그 순간이 신청
  버튼을 급하게 누르는 순간이다. 총원은 전광판 라벨이 말하므로 공은 분위기만 맡는다.
