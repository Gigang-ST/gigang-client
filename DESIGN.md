...
보이면 되는 자리(기록 자랑 작성·다건 마일리지런)는 `size="half"`로 면적을 1/4로 줄인다. 사진이 그
...
| RecordFlexFeed | `record-flex-feed.tsx` | 기록 자랑 — 사진 정사각 격자, 세로 2칸씩 가로로 흐름. 끝에 닿으면 더 불러오기(오프셋). **사진 없는 기록은 아예 안 뜬다** |
| RecordFlexCreateDialog | `record-flex-create-dialog.tsx` | 기록 작성 — 사진·한마디·날짜(수치 없음). 필수는 사진·날짜, 한마디는 선택 |
...
- **"팻말·꽂기" 어휘는 목표 한마디 존 전용이다**: 코스변 손팻말(`PledgeSigns`)에만 쓴다.
  기록 자랑은 인스타형 사진 격자라 코스도 팻말도 없으므로 **"올린다/자랑한다"**로 말한다.
...
- **운동기록은 사진이 있어야 선다**: 이 지면의 기록은 **수치를 겨루는 게 아니라 친목·응원**이라
  매개가 사진이다. 그래서 기강이야기 작성 폼엔 **거리·종목이 없고**(사진·한마디·날짜 셋뿐),
  사진 없는 기록은 지면에 아예 안 뜬다. **필수는 사진과 날짜뿐 — 한마디는 선택**이다
...
- **수치가 필요하면 마일리지런에서 올린다**: 마일리지런은 반대로 **수치가 본체**라 사진이 선택값이다.
  ...
  (기강이야기=필수 / 마일리지런=선택. 한마디는 양쪽 다 선택). 사진 필드 아래 빨간 안내문
  ("사진 추가 시, 기강이야기의 운동기록에도 등록돼요")으로 **누르기 전에** 알린다 — 공개는 되돌리기
...
- **리드 슬롯은 kicker / body / footer 3밴드로 고정**(`story-lede.tsx`): 슬롯마다 자기 레이아웃을
  ...
  - **다섯 슬롯 모두 응원을 받는다.** 운동기록의 응원 대상은 **글이 아니라 올린 사람**이다 —
    ...
    `actv` 검증은 활동지수 랭킹 / 목표 팻말 / **기록 자랑 작성자**(`story-posts` 캐시) 세 출처를
