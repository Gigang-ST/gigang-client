...
번지기만 한다. 그래서 이 존을 쓰는 곳은 셋이다: 프로필 카드 상단(프레임·칭호 이펙트),
기강이야기 팀 펄스 밴드(`--pulse-neon` 심전도), 그리고 **기강의 전당 챔피언 띠**
(프레임·칭호 이펙트 — §기강의 전당). **glow가 없는 요소를 어둡게 만들려고
...
쓴다. **금색은 없다** — 1위는 board 띠로 올라가 `title-effect-gold` 제호와 앰버 계기 어법을 쓴다.
...
- **제호는 `RECORD HOLDER`(트레일만 `TOP INDEX`) — "CHAMPION"이 아니다.** 이 사람은 어떤 경기에서
  ...
  제호가 달라도 어색하지 않다.
- **제호와 기록은 금·앰버로 반짝인다.** 제호는 칭호 이펙트의 `title-effect-gold`를 그대로 쓴다 —
  같은 앱에서 "가장 귀한 것"을 말하는 표현이 이미 있는데 새 어휘를 만들 이유가 없다.
  기록 숫자는 등번호와 같은 계기 숫자라 `board-amber`. 띠에 앰버가 한 단어뿐이면 그냥 검은 줄로 읽힌다.
...
- **1위는 board 띠(`BoardBand`)로 올라간다.** 랭킹 행에 붙는 칭호 프레임 glow는 흰 지면에서 빛나지
  않고 뿌옇게 번지기만 한다(§Board 토큰) — 띠가 그 조건을 만든다. 화면 좌우 끝까지 붙이는 것도
  같은 어법이다(팀 펄스 밴드와 동일: 흰 지면 위에 뜬 검은 상자는 광고 배너로 읽힌다).
  **점등(flicker·cone)은 켜지 않는다** — 탭은 매번 들어오는 자리라 진입할 때마다 깜빡이면
  피곤하다(프로필 편집판이 플리커를 끈 것과 같은 이유).
...
- **내 기록 판독선은 띠 밑단에 간격 없이 붙는다.** 이 줄은 순위축 위에 놓인다(1위 다음에 내가 오고
  ...
  판독선으로 층을 뗀다. **격차(`1위까지 +14:03`)가 붙어야** 이 줄이 "N위 행"이 아니라 나와 전당
  사이를 재는 계기가 된다 — 바로 위가 그 계산의 기준이다. 누르면 목록의 내 행으로 보낸다.
  기록이 없으면(로그인 상태) 같은 자리가 **등록 진입점**이 된다.
- **목록 카드에서 대회명을 뺐다.** 반칸 카드는 375px에서 폭 163px인데 정보를 다섯 개 담느라
  대회명 10px·기록 11px까지 내려갔다(Micro 최소치 11px 아래). 대회명은 행을 눌러 여는 카드의 몫이다.
  뺀 덕에 이름 13.5px·기록 16px. 1위가 띠로 갔으니 목록은 2위부터라 카드가 헐거워진 것도 같이 작용한다.
