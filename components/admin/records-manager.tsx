...
import type { RaceRecordItem } from "@/lib/queries/admin-data";

export function RecordsManager({
  initialRecords,
}: {
  initialRecords: RaceRecordItem[];
}) {
  const [records, setRecords] = useState<RaceRecordItem[]>(initialRecords);
  ...
  const startEdit = (record: RaceRecordItem) => {
    setEditingId(record.id);
    setEditTime(formatTime(record.record_time_sec));
  };

  const handleUpdate = async (record: RaceRecordItem) => {
