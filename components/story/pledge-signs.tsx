 ...
 *
 * 각오는 만료 없이 쌓이고, 가로로 밀어 지나가는 게 곧 코스를 달리는 감각이다. 팻말을 탭하면
 * 그 사람 프로필이 열리고, "내 팻말 만들어 꽂기"로 로그인 멤버 누구나 한 줄 다짐을 남긴다.
 *
 * **각오에는 시계가 없다.** 24시간 규칙은 이 존이 아니라 종이비행기 한마디(`MessagePlanes`)
 * 것이다 — 각오는 "앞으로 이렇게 하겠다"는 다짐이라 하루 만에 지울 이유가 없고, 1인 1개라
 * 새로 쓰면 이전 것이 내려간다(`createPledge`의 del_yn 스윕).
 *
...
export function PledgeSigns({
  ...
  return (
    <section className="flex flex-col">
      ...
      {myMemId && (
        <div className={hasPledges ? "px-6 pt-10" : "px-6 pt-5"}>
          <button
            type="button"
