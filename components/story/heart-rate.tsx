 ...
 * (50~200)을 받아, 높을수록 파형을 빠르고 크게, 낮을수록 느리고 평평하게 그린다.
 * 50 근처는 거의 평선(flatline)이라 "죽어 있다"가 한눈에 읽힌다.
 *
...
/** BPM 범위 — team-pulse.ts와 같은 값(50~200). 이 안에서 모양·속도를 보간한다 */
const BPM_MIN = 50;
const BPM_MAX = 200;
 ...
 * `amp`로 전체 진폭을, `flat`이면 거의 눌러 평선에 가깝게(실종 단계).
 *
...
function beatPath(intensity: number): string {
  ...
  // 확실히 뛴다(0.30). 완전 평선은 "죽음"이라 위험 신호지, 활동이 적은 것과는 다르다.
