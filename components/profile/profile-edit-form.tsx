...
export function ProfileEditForm({
  ...
  member: MemberData;
  initialNearStnNm: string | null;
}) {
  const router = useRouter();
  const [avatarState, setAvatarState] = useState<AvatarState>({
    kind: "current",
  });
  // 가까운 역은 mem_onbd_prf 위성 테이블 소관이라 RHF 폼과 별도 액션으로 저장한다.
  const [nearStnNm, setNearStnNm] = useState<string | null>(initialNearStnNm);
  ...
  const stationDirty = nearStnNm !== initialNearStnNm;
  ...
  const saveDisabled =
    isSubmitting ||
    compressing ||
    (!isDirty && avatarState.kind === "current" && !stationDirty);
  ...
  return (
    <div className="flex flex-col gap-6 px-6 pb-6 pt-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Caption>코스·모임 안내에 참고해요.</Caption>
      </div>
