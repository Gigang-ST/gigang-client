...
export function RaceListView({
	...
	const [regCounts, setRegCounts] =
		useState<Record<string, number>>(initialRegCounts);
	...
	const loadMyRegsForIds = async (
		...
		const { data: myRegs } = await supabase
			.from("comp_reg_rel")
			.select(
				"comp_reg_id, mem_id, prt_role_cd, comp_evt_id, crt_at, team_comp_plan_rel!inner(comp_id), comp_evt_cfg(comp_evt_type)",
			)
			.eq("mem_id", memberId)
			.eq("vers", 0)
			.eq("del_yn", false)
	...
	const loadPastChunk = async (before: string) => {
		setPastLoading(true);
		try {
			const { list, nextBefore } = await getPastGigangCompetitions(
				before,
				PAST_MONTHS_CHUNK,
			);
	...
	const loadMorePast = () => {
		const before =
			pastNextBefore ??
			pastCompetitions.reduce(
				(min, c) => (c.start_date < min ? c.start_date : min),
				pastCompetitions[0]?.start_date ?? "",
			);
		if (!before) return;
		loadPastChunk(before);
	};
	...
	const allCompetitionIds = useMemo(
		() => [
			...new Set([
				...
				...(pastOpen ? pastCompetitions.map((c) => c.id) : []),
			]),
		],
	...
	useEffect(() => {
		...
		allCompetitionIds,
		memberStatus.status,
		memberStatus.status === "ready" ? memberStatus.memberId : null,
	]);
	...
	const createRegistration = async (
		...
		if (
			payload.role === "participant" &&
			eventType &&
			compEvtTypeContainsHangul(eventType)
		) {
			return {
				ok: false as const,
		...
		const ensured = await ensureTeamCompPlanRel(
			supabase,
			teamId,
			competitionId,
		);
	...
	const deleteRegistration = async (
		...
		const { error } = await supabase
			...
			.delete()
			.eq("comp_reg_id", registrationId)
			.eq("mem_id", memberStatus.memberId);
						...
						>
							...
							{group.items.map((comp) => {
								...
								const isRegistered = Boolean(
									registrationsByCompetitionId[comp.id],
								);
													...
													{comp.sport && SPORT_LABEL[comp.sport] && (
														<span
															className={cn(
																...
																SPORT_LABEL[comp.sport].className,
															)}
														>
			...
			{tab === "gigang" && pastOpen && (
				<div className="flex flex-col gap-4 px-6 pb-6">
					...
					pastNextBefore === null ? (
						...
						<p className="py-8 text-center text-sm text-muted-foreground">
							해당 기간 지난 대회가 없습니다. 아래에서 이전 구간을 불러올 수
							있습니다.
						</p>
					) : (
						(() => {
