-- 리드 대회 슬롯 2종의 대회명을 대회 상세로 딥링크(/schedule?comp=<short_id ?? comp_id>)하기 위해
-- get_team_story_feed 의 records 에 comp_id·short_id 를 싣는다.
--
-- recent_recs 는 이미 comp_mst(c) 를 조인하고 있어 **새 조인·필터 변경이 없다** — payload 에만 얹는다.
-- 다가오는 대회(races)는 이미 comp_id·short_id 를 내려주고 있어 손대지 않는다.
-- 클라(StoryRecord.comp_id?/short_id?)는 옵셔널이라 구버전 RPC 에서도 링크만 빠지고 이름은 그대로 나온다.
