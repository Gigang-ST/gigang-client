...
-- 기강 전광판 피드 RPC v2
--   ① actv_rank — 기강활동지수 랭킹(전체 누적, 상위 10명). pt_txn_hist 원장 합산.
--      "기강포인트"는 히든 운영이라 반환 키에 point/pt 문자열을 쓰지 않고 actv_score로만 노출한다
--      (설계서 8절·11절 — 활동지수 형태로는 공개 확정).
--   ② week_stat — 이번 주(KST 월요일 시작 ~ now) 크루 합계 단일 오브젝트.
--   ③ month_rank는 5명 유지, records는 8→10건으로 확대(프론트 "더보기" 한도 변경).
--
-- v1(20260722110000) 대비 변경만 담았고, CTE 구조·조인 조건(vers=0, del_yn=false,
-- mem_st_cd='active', KST 타임존 처리)은 그대로 답습한다.
...
week_start AS (
  ...
  SELECT date_trunc('week', now() AT TIME ZONE 'Asia/Seoul') AS w
),
newbies AS (
...
COMMENT ON FUNCTION public.get_team_story_feed(uuid, uuid) IS
  '기강 전광판 피드 — 신규가입·최근기록·다가오는대회·이달의참가왕(month_rank)·기강활동지수랭킹(actv_rank)·이번주통계(week_stat) + 리액션 카운트. p_mem_id가 있으면 내 리액션을 함께 내린다. actv_rank는 pt_txn_hist 전체 누적 합산(월 한정 아님), 반환 키에 point/pt 문자열을 쓰지 않는다(기강 포인트는 히든 운영).';
