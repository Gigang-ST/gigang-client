-- 활동지수 랭킹에 프로필 부품 데이터 추가 (intro·칭호·러닝프로필·개인 최고기록·이번 달 참석/기록)
--
-- 배경: /story 리드 §④ "이번 달 기강 잡는" 슬롯이 대표 1명을 "프로필 부품 조합"으로
-- 그린다(아바타+이름 + 칭호·소개·개인최고기록·러닝프로필 + 오른쪽에 이번 달 참석·기록 횟수).
-- newbies가 이미 쓰던 서브쿼리 패턴(intro_txt·primary_title·running_profile)을 actv_rank에
-- 그대로 복제하고, 여기에 **개인 최고기록 = 가장 긴 거리 종목 1건**과 **이번 달 참석/기록
-- 등록 횟수**(mth_attd_cnt·mth_rec_cnt)를 새로 얹는다.
--
-- 거리 판정: rec_race_hist·comp_evt_cfg에 거리 컬럼이 없어 comp_evt_type 문자열에서 유도한다.
--   · 'NNK'(10K~200K, "35K DUO" 포함)는 정규식 ^([0-9]+)K로 숫자를 뽑는다.
--   · 명명형은 상수 매핑: FULL=42 · HALF=21 · OLYMPIC/TRIATHLON_OLYMPIC=51 ·
--     TRIATHLON_HALF=113(70.3) · 100M=161.
--   · 파싱 불가(GRANFONDO·MEDIOFONDO·TRAIL·5PEAKS·2PEAKS·"50")는 거리 0으로 후순위로 밀되,
--     그런 종목만 있는 멤버도 rec_time_sec 최소(=최고 기록)로 폴백해 항상 1건은 나온다.
--
-- 변경은 두 곳뿐이다: (1) actv_rank CTE에 tm.team_mem_id 추가(칭호 서브쿼리가 team_mem_id로
-- 조인), (2) 최종 SELECT의 'actv_rank' jsonb에 부품 필드 추가. 나머지 CTE·SELECT는
-- 20260724172355(actv_rank_all_members)에서 그대로 가져왔다(CREATE OR REPLACE라 전체 본문 재작성).
...
actv_rank AS (
  ...
  -- team_mem_id를 함께 들고 나온다 — 칭호(mem_ttl_rel)·배지·프레임 서브쿼리가 team_mem_id로 조인한다.
...
SELECT jsonb_build_object(
  ...
  'actv_rank', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      ...
      -- 이번 달 참석 수 — month_rank CTE와 같은 집계(gthr_mst.stt_at 이번 달, KST)를 개인별로.
      'mth_attd_cnt', (SELECT count(*) FROM public.gthr_attd_rel ga
                       ...
                       WHERE ga.mem_id = ar.mem_id
                         ...
                         AND gm.stt_at < now()),
      -- 이번 달 기록 수 = 대회 기록 + 마일리지런 + 순수 기록 자랑. 셋을 합산하되 **이중 집계를
      -- 피한다**: 마일리지런 기록(후기 있으면)은 트리거(post_sync_from_mlg_act)가 post_mst에
      -- src_enm='mlg_auto'로 자동 복사하므로, post는 mlg_auto를 빼야 마일리지런이 두 번 안 세진다.
      -- 기간은 각 원장의 활동일(race_dt·act_dt) 기준 이번 달.
      'mth_rec_cnt', (
      ...
      -- 개인 최고기록 목록 — **종목(comp_evt_type)별 최고기록 1건씩**을 거리 긴 순으로 상위 4종.
      -- 자리가 남아 풀·하프·10K 등을 나란히 보여준다(단건일 때보다 그 사람 실력이 입체적으로).
      -- 각 종목 안에서는 rec_time_sec 최소(=최고), 종목 간에는 거리 내림차(comp_evt_type 유도).
      'best_records', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
               ...
               ORDER BY top.km DESC, top.rec_time_sec ASC)
        FROM (
          -- 종목당 최고기록 1건만 남기고, 거리 긴 순 상위 4종. jsonb_agg에는 LIMIT을 못 걸어
          -- (집계 함수라) 여기 서브쿼리에서 정렬·LIMIT을 끝낸 뒤 바깥에서 agg한다.
          SELECT br.sport, br.evt, br.rec_time_sec, br.race_nm, br.race_dt, br.km
          FROM (
          ...
          ORDER BY br.km DESC, br.rec_time_sec ASC
          LIMIT 4
        ) top
      ), '[]'::jsonb)
      ) ORDER BY ar.rn)
