-- 리드 "다가오는 대회" 슬롯을 종목별 참가자 나열(B안)로 바꾸기 위해,
-- get_team_story_feed 의 races.runners 에 종목(evt)을 싣고 **참가자(participant)만** 담는다.
--
-- 기존 runners 는 role 무관 전원을 jsonb_agg(DISTINCT ...)로 담았다. 나열은 참가자만 보여주고
-- 종목별로 묶으므로, runners 를 participant + comp_evt_type 포함 + 종목·이름 순 정렬로 바꾼다.
-- reg_cnt(전원 카운트)는 다른 표시와의 호환을 위해 그대로 둔다 — UI 카운트는 runners 기반으로 쓴다.
--
-- 직전 마이그(20260726140000_story_feed_pledge_profile)의 pledges 프로필 확장은 그대로 유지한다.
...
upcoming AS (
  SELECT c.comp_id, c.short_id, c.comp_nm, c.stt_dt,
         ...
         -- 참가자(participant)만 종목·이름 순으로 나열 — 리드 슬롯이 종목별로 묶어 그린다.
