-- 리드 §④ 활동지수 슬롯의 개인 최고기록 정렬을 **풀코스 > 하프 > 10K** 우선으로 바꾼다.
--
-- 기존(20260726120000)은 순수 거리 내림차(km DESC)라 울트라(55K)·철인(113)이 풀코스(42)보다
-- 위로 와, 대표(best_records[0])가 울트라가 되어버렸다. 러닝 대표 종목 순서(풀→하프→10K)를
-- 명시적 pb_rank로 최우선에 두고, 그 외 종목은 뒤로 밀되 거리 긴 순으로 유지한다.
--   pb_rank: FULL=0, HALF=1, 10K=2, 그 외=3
--   정렬: pb_rank ASC → km DESC → rec_time_sec ASC
-- 기록이 없으면 best_records는 그대로 '[]' (프론트가 조각을 안 그리거나 "기록 없음" 문구).
--
-- best_records 서브쿼리만 바뀌었다 — 나머지 CTE·SELECT는 20260726120000과 동일
-- (CREATE OR REPLACE라 전체 본문 재작성).
