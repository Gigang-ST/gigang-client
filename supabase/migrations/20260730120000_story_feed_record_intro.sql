-- 전광판 리드 "결승선을 넘다"(완주기록) 슬롯에 소개 한마디를 싣는다.
--
-- 이 슬롯은 사람을 세우는 리드 넷(새 얼굴·활동지수·목표 한마디·완주기록) 중 유일하게
-- 한마디가 빠져 있었다. 프로필 부품을 붙인 게 #454인데 그때 칭호·배지·프레임만 골라
-- 실었고, 화면에도 얼굴 오른쪽이 이름·칭호로 끝나 그 아래가 비어 있었다.
--
-- `recent_recs`는 이미 `team_mem_rel`을 INNER JOIN하고 있어 컬럼 하나만 더 끌어오면 된다.
-- newbies·recent_pledges가 같은 자리에서 쓰는 것과 **똑같은 모양**이라 새로 설계한 게 없다.
--
-- 나머지 존(newbies·races·actv_rank·pledges)은 한 글자도 바꾸지 않았다.
...
SELECT jsonb_build_object(
  ...
  'records', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      ...
      'badge_effect',rc.badge_effect,'frame_cd',rc.frame_cd,'intro_txt',rc.intro_txt,
      'primary_title',(SELECT jsonb_build_object(
                         'ttl_nm',t.ttl_nm,'ttl_desc',t.ttl_desc,'desc_visibility',t.desc_visibility)
                       FROM public.mem_ttl_rel mt
                       INNER JOIN public.ttl_mst t
