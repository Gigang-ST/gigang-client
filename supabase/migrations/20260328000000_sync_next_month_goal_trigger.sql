-- 기록 변경 시 다음 달~종료월까지 목표를 연쇄 갱신하는 트리거
...
begin
  ...
  -- 기록이 속한 월부터 종료월 전월까지 연쇄 갱신
  v_current_month := v_activity_month;

  loop
    v_next_month := (v_current_month + interval '1 month')::date;

    -- 종료월을 넘으면 종료
    if v_next_month > v_end_month then
      exit;
    end if;
    ...
    from activity_log
    where participation_id = v_participation_id
      and activity_date >= v_current_month
...
-- 트리거 재생성
drop trigger if exists trg_sync_next_month_goal on activity_log;
