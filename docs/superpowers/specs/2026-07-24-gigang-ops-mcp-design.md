...
| `update_my_activity` | `act_id` + 위 필드(**전부 선택** — PATCH) | act_id, before, after, month_after | 본인 |
...
- **수정은 PATCH 다 — `act_id` 말고 전부 선택이고, 안 준 것은 건드리지 않는다.** 두 번에 걸쳐 여기까지 왔다.
  1. 한때 `multipliers` 만 "생략=유지"였고 `review`·`elevation_m` 은 "생략=null/0 으로 설정"이라 **같은 호출 안에서 방향이 반대**였다. 거리 오타만 고치러 온 호출에서 후기가 조용히 날아갔고, 고도는 러닝 마일리지(거리 + 고도/100)에 직접 들어가 **숫자까지 틀어졌다.**
  2. 그 뒤에도 `act_dt`·`sport`·`distance_km` 는 required 로 남아, "고도만 고치기"가 셋을 다시 안 적으면 거부됐다 — description 은 "고치려는 것만 보내라"고 하는데 실제로는 셋을 늘 다시 적어야 해서 **말과 동작이 갈렸다.** 문구를 약하게 고치는 대신 약속을 사실로 만들었다.
  - 지우려면 명시한다: `review=null` · `elevation_m=0` · `multipliers=[]`. 아무것도 안 주면 안전한 no-op 다.
  - **등록(`log_my_activity`)은 그대로 required 다** — 이을 기존 값이 없어 날짜·종목·거리 없이 만들 수 있는 기록이 아니다. PATCH 규약은 수정에만 성립한다.
