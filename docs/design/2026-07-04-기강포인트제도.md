...
| 운동기록(기록 자랑) 등록 | **+3** | **뛴 날(`act_dt`)** | 기강이야기 직접 등록. **1일 제한 없음**. 마일리지런에 사진을 붙여 유입된 건(`mlg_auto`)은 **차액 +1만** 얹어 합계 3점 (§2.1) |
| 마일리지런 월 목표 달성 | **+10** | 해당 목표 월 1일 | 실시간 판정 — 누적 ≥ 목표가 *처음* 되는 순간 적립 (§5.4) |
| 정보 등록 | **+5** | 작성일 | `sch_post_mst`. 상한 없음 |

### 2.1 운동기록 — 두 경로가 같은 3점이 되도록 (2026-07-28 추가)

기강이야기 기록 자랑은 **직접 등록**과 **마일리지런 유입** 두 경로가 있는데, 결과물(사진 한 장이
지면에 서는 것)이 같으므로 **합계도 같아야 한다**.

| 경로 | 적립 내역 | 합계 |
|------|-----------|------|
| 직접 등록(`manual`) | `post_record` +3 | **3** |
| 마일리지런 + 사진(`mlg_auto`) — 그날 **첫** 건 | `mlg_record` +2, `post_record` **+1** | **3** |
| 마일리지런 + 사진(`mlg_auto`) — 그날 **둘째~** | `mlg_record` 0, `post_record` **+3** | **3** |
| 마일리지런 + 사진 없음 | `mlg_record` +2 | 2 (기강이야기에 안 뜬다) |

마일리지런은 원천(`evt_mlg_act_hist`)에서 이미 +2를 받는다. `post_mst`에 3점을 그대로 더 주면
**5점이 되어 이중 적립**이 되므로 차액(+1)만 얹는다.

**단, 원천 `mlg_record`는 하루 1건만 +2다**(§마일리지런 룰). 그래서 차액을 상수 +1로 박으면
같은 날 둘째 기록부터는 `0 + 1 = 1점`이 되어 계약이 깨진다(실측으로 확인). 금액은
`pt_post_record_amt_for(src, mem_id, aply_dt)`가 **그날 이미 유입분을 받았는지** 보고 정한다 —
첫 건이면 차액(+1), 둘째부터는 전액(+3). 어느 쪽이든 기록 하나당 합계는 3이다.

> 판정 기준을 "그날 `mlg_record`가 있는가"로 두면 안 된다. 원천은 하루 한 번만 잡히므로
> 둘째·셋째 기록도 전부 "원천 있음"으로 읽혀 다 같이 +1만 받는다. 원천이 아니라 **이 활동
> 자신의 그날 적립 이력**(`pt_post_record_mlg_auto_cnt`)을 봐야 "첫 건만 차액"이 성립한다.

**사진 유무는 포인트 트리거가 판정하지 않는다.** `post_sync_from_mlg_act`가 사진이 있을 때만
`mlg_auto` row를 만들고 사진을 떼면 `del_yn=true`로 내리므로, `trg_pt_post_mst`의 INSERT/UPDATE
분기가 곧 "사진이 붙었나/떨어졌나"다.

**회수는 UPDATE에서 잡는다** — 기록 자랑 삭제는 DELETE가 아니라 `del_yn=true` 소프트삭제다.
DELETE 트리거만 달면 앱이 실제로 쓰는 삭제 경로를 통째로 놓친다.

1일 제한은 두지 않았다(사용자 결정) — 사진 필수라 쪼개기 파밍이 어렵다. 그래서 마일리지런처럼
(멤버, 날짜) net 판정을 쓰지 않고 `ref_id`(post_id) 건별로 적립·회수한다.
