...
/**
 * 내역 시트도 **이번 달만** 본다 — 조회 구간은 `getActvMonthRange()` 하나뿐이다.
 *
 * 한때 3개월치로 넓히려다 되돌렸다. 활동량은 매달 1일에 0으로 리셋되는 제도라
 * (§ACTV_HELP_TEXT) "이번 달"이 곧 이 지표의 단위다. 범위를 넓히면 목록엔 지난달 줄이
 * 보이는데 하단 합계는 랭킹과 맞추려고 이번 달만 더해야 해서, 같은 시트 안에 기준이 둘이
 * 생긴다 — 달 구획·"합계는 7월 몫" 각주로 수습해야 할 만큼. 목록과 합계가 같은 구간이면
 * 설명이 아예 필요 없다.
 */

/** 활동량 도움말 본문 — 물음표 팝오버와 빈 상태가 같은 문구를 공유한다 */
export const ACTV_HELP_TEXT =
  "모임에 나오고, 대회를 뛰고, 기록을 올릴 때마다 활동량이 쌓여요. 매달 1일에 0부터 다시 시작해요.";
