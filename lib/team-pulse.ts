...
/**
 * 활동량 가중치 — **모임 참석 1 : 기록 0.25**.
 *
 * 예전엔 1:1로 그냥 더했는데, 두 값은 성격도 빈도도 다르다.
 *
 * - **기록(`rec_cnt`)은 혼자서 얼마든지 올릴 수 있다.** 깅스타그램은 사진만 있으면 1일 제한이
 *   없어서(§DESIGN — 사진 필수라 쪼개기가 어렵다는 이유로 제한을 안 뒀다) 한 사람이 하루에
 *   여러 건을 올리면 크루 전체 심박이 따라 뛴다. 참석은 실제로 나가야 1이 오른다.
 * - **크루 취지가 모임 중심이다.** 포인트 제도(`docs/design/2026-07-04-기강포인트제도.md`)도
 *   벙 참석 10점 / 깅스타그램 3점으로 참석을 무겁게 둔다 — 심박도 같은 방향이어야 한다.
 *
 * 0.25는 prd 실측으로 고른 값이다. 참석이 평소인데 기록만 20건 터진 주는 `꾸준한 페이스`에
 * 머물고(파밍으로는 심박이 안 터진다), 참석이 실제로 늘어난 주는 `심장 폭발`이 나온다.
 * 0.5로 올리면 기록 20건짜리 주가 `심장 폭발`이 되어 그 경계가 무너진다.
 *
 * **마일리지런은 일부러 안 넣는다.** 프로젝트성 기간 이벤트라 종료되는 달에 분자가 통째로
 * 꺼져 심박이 이유 없이 죽는다. 사진을 붙인 마일리지런은 어차피 `mlg_auto`로 깅스타그램에
 * 유입되므로(§DESIGN — `post_sync_from_mlg_act`) 기록 항에 이미 반영돼 있다.
 *
 * 모임 개수(`gthr_cnt`)는 여전히 제외 — 규모를 안 담는다(모임 1개에 1명이든 11명이든 1).
 */
const ATTD_WEIGHT = 1;
const REC_WEIGHT = 0.25;

/** 이번 주 활동량 = 참석 연인원 + 새 기록 × 0.25 (§`ATTD_WEIGHT`/`REC_WEIGHT`) */
function activityOf(week: TeamWeek): number {
  return week.attd_cnt * ATTD_WEIGHT + week.rec_cnt * REC_WEIGHT;
}
 ...
 *
 * 분자·분모에 쓰는 활동량은 **참석 1 : 기록 0.25**로 가중한다(`activityOf`) — 혼자 연타할 수
 * 있는 기록이 실제로 모여야 오르는 참석과 같은 무게를 갖지 않게.
 *
...
export function getTeamPulse(weeks: TeamWeek[]): TeamPulse {
  ...
  // 임계값은 **가중치가 적용된 활동량** 기준이다 — 참석 10명이면 예전과 똑같이 최상이지만,
  // 기록만으로 채우려면 40건이 필요하다. 이건 의도다(위 `REC_WEIGHT` 근거와 같은 방향).
  if (baseline <= 0) {
