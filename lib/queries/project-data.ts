...
/** 이벤트 누적 목표 (startMonth ~ endMonth 포함) — 당월이면 60s, 과거월이면 1시간 캐시 */
export const getEventGoalsCumulative = cache(
  async (evtId: string, startMonth: string, endMonth: string) => {
    ...
    const isCurrentMonth = endMonth >= currentMonthKST();
...
/** 이벤트 누적 활동 로그 (startDate ~ endMonth 1일 exclusive) — 당월이면 60s, 과거월이면 1시간 캐시 */
