...
export { SCH_POST_TYPES, schPostTypeLabels, schPostTypeInlineLabel } from "./schedule-types";
export type { SchPostType } from "./schedule-types";

export const createSchPostSchema = z.object({
  team_id: z.string().uuid(),
  sch_nm: z.string().min(1, "제목을 입력해 주세요.").max(100, "제목은 100자 이내로 입력해 주세요."),
  post_type: z.enum(["race_entry", "sale", "session", "general"] as const, { message: "유형을 선택해 주세요." }),
